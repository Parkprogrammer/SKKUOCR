...
def evaluate_accuracy(recognizer, op2val, converter, test_loader, device="cuda"):
    ...
    with torch.no_grad():
        for imgs, gt_texts in test_loader:  # collate_eval → (imgs: Tensor, gt_texts: List[str])
            B, C, H, W = imgs.size()
            imgs = imgs.to(device)

            # ① Tensor → Numpy 이미지 리스트로 변환
            #    (datasets.py evaluate_dataset 코드와 동일)
            img_list: List[np.ndarray] = (imgs[:, 0] * 255).byte().cpu().numpy()  # (B, H, W)
            img_list = [img for img in img_list]

            # ③ recognize_imgs() 로 예측 수행 ([(pr_txt, conf), ...])
            recognizer.to(device)
            preds = recognize_imgs(img_list, recognizer, converter, op2val)

            # ④ GT∙PR 비교 (클린된 문자열 기준)
            #    evaluate_dataset에서 사용하는 방식과 동일하게,
            #    공백∙특수기호 제거 후 소문자로 맞춘 뒤 비교

...
import torch
torch.backends.cudnn.enabled = False          # cuDNN RNN 우회
...
# ───── 2. 라벨 클린 & 허용 문자 필터 ❶ ──────────────────────────────────────
CHAR_MAP = {'λ':'lambda','√':'sqrt','≤':'<=','≥':'>=','‖':'||','⋯':'...','∈':'in','∑':'sum'}

ALLOW = set(                  # *** 읽고자 하는 문자 집합 ***
    "0123456789"
    "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
    "가나다라마바사아자차카타파하"          # 자주 쓰이는 초성
    "ㅏㅑㅓㅕㅗㅛㅜㅠㅡㅣ"                # 모음
    " .,!?~&%+-*/=_()[]{}<>‘’“”…·|@#^$"   # 기본 기호·공백
)
UNK = "▯"   # 허용 밖 글자는 모두 이 토큰으로

def clean_label(s:str)->str:
    s = s.replace('\n',' ').strip()
    s = s.translate(str.maketrans(CHAR_MAP))
    s = " ".join(s.split())
    # ---------- 허용 문자 필터 ----------
    s = "".join(c if c in ALLOW else UNK for c in s)
    # UNK만으로 이뤄졌으면 학습 가치 없음 → 빈 문자열 반환
    return "" if set(s) == {UNK} else s

# ───── 3. NormalizePAD · Dataset (동일) ─────────────────────────────────────
class NormalizePAD:
...
class OCRFinetuningDataset(Dataset):
    ...
    """
    def __init__(self, image_root, label_root, transform=None, max_samples=-1):
        ...
        for cat in cats:
            ...
            if 0<max_samples<len(imgs): imgs=imgs[:max_samples]
...
def _charset(conv)->str:
    ...
    if hasattr(conv,"characters"):
        ch = conv.characters; return "".join(ch) if isinstance(ch,(list,tuple)) else ch
    raise AttributeError

def build_converter(train_ds, base_conv):
    base = _charset(base_conv)
    freq = Counter()
    for s in train_ds.samples: freq.update(s["label"])
    extra = sorted({c for c in freq if c not in base and c != UNK})[:50]  # ★ hard-limit 50
    charset = base + "".join(extra) + UNK
    print(f"[INFO] vocab 길이: {len(charset)} (추가 {len(extra)}자, UNK 포함)")
...
def train(recog, conv, train_ds, test_ds, out_dir,
    ...
    for ep in range(1, epochs+1):
        ...
        for b,bt in enumerate(bar):
            imgs=bt["image"].to(device); logits=recog(imgs)
            tg_all,len_all = conv.encode(bt["label"])
...
def main():
    ...
    train_ds = OCRFinetuningDataset(img_train, lbl_train, tfm)
    test_ds  = OCRFinetuningDataset(img_test , lbl_test , tfm)
    ...
    train(recog, conv, train_ds, test_ds, "finetune_results",
          epochs=5, device="cuda" if torch.cuda.is_available() else "cpu",
